...
export class MailService {
    constructor(private mailerService: MailerService, private productService: ProductsService) { }
    ...
    confirmationHTML = async (customer, order) => {
        ...
        for (const i in order.products) {
            const product = await this.productService.findOne(order.products[i].productId);
            products.push({ name: product.name, quantity: order.products[i].quantity });
        }
